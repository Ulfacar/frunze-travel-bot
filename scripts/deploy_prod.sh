...
if healthy; then
    # PROD_HEALTHY, но НЕ PROD_FUNCTIONALLY_VERIFIED: /health говорит только «процесс жив».
    # Обещать больше, чем проверено, нельзя — это класс ошибки, ради которого написан
    # `docs/protocol.md`. Функцию проверяет человек или агент, вызывая её в контейнере.
    echo "== /health отвечает, код развёрнут: $NEW"
    echo "== ФУНКЦИОНАЛЬНО НЕ ПРОВЕРЕНО: вызови изменённую функцию в контейнере и сверь вывод"
    exit 0
fi
