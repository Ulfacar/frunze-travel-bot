...
SCENARIOS: dict[str, list[tuple[str, str]]] = {
    ...
    # Клиент вернулся к первому варианту: каждая смена — событие, но одно и то же
    # событие не должно прийти дважды.
    "back_and_forth": [
        ("1", "Здравствуйте! Турция, Анталья, всё включено, вылет из Бишкека, "
              "с 5 по 11 сентября, двое взрослых, бюджет до 2000 долларов"),
        ("2", "А давайте лучше ОАЭ, Дубай"),
        ("3", "Хотя нет, всё-таки вернёмся к Турции, Анталья"),
        ("4", "Да, Анталья, всё верно"),
    ],
    # Шумовой тест закона 5: клиент дёргает бюджет и даты, направление не трогает.
    # Правильный ответ — НИ ОДНОГО уведомления.
    "noise_only": [
        ("1", "Здравствуйте! Турция, Анталья, всё включено, вылет из Бишкека, "
              "с 5 по 11 сентября, двое взрослых, бюджет до 2000 долларов"),
        ("2", "Бюджет давайте до 2500"),
        ("3", "А даты сдвинем на 12-18 сентября"),
        ("4", "И нас будет трое, ещё ребёнок 6 лет"),
        ("5", "Бюджет тогда до 3000"),
    ],
    "offer_then_change": [
