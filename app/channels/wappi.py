...
# Эхо ответа менеджера. `outgoing_message_phone` — ответ с телефона (так работают туровые);
# `incoming_message` + is_me — старая форма того же события, оставлена как страховка.
#
# `outgoing_message_api` слушаем с 11.08.2026. Раньше он был выключен как «эхо отправок
# самого бота» — и это оказалось половиной правды: поддержка Wappi подтвердила, что ответ
# оператора ИЗ БИТРИКСА идёт по цепочке «Открытая линия → Wappi API → WhatsApp» и приходит
# ровно этим типом. Под ним смешаны наши собственные реплики и ответы визовых менеджеров,
# которые работают в Контакт-центре. Отбросив тип целиком, мы теряли вторые: замер за 14
# дней — 36 сообщений менеджеров по визам против 845 у туров при равном потоке клиентов.
#
# Своё эхо отсекается двумя рубежами (см. `main._handle_manager_echo`): `is_own` по id
# провайдера и сверка текста с последними репликами бота в этом же диалоге. Второй рубеж
# обязателен: `is_own` живёт 15 минут в памяти процесса, а принятая за менеджера собственная
# реплика означает автоперехват — бот замолчал бы на этом клиенте.
_ECHO_WH_TYPES = ("outgoing_message_phone", "outgoing_message_api", "incoming_message")
