...
def _tours_search_report(found, *, cards_mode: bool = False,
    ...
    if found.fallback_departure:
        head.append(
            f"ВАЖНО: по запросу клиента туров не нашлось, это варианты с вылетом "
            f"ИЗ ДРУГОГО ГОРОДА ({found.departure}). Обязательно предупреди клиента — "
            "не выдавай их за вылет из того города, который он называл."
        )
    if found.relaxed:
        # Выдать чужие условия за условия клиента — тот же обман, что и выдуманная
        # причина отказа. Замер 11.09: клиент просил неделю, чартер летит на 5 ночей —
        # показать их молча значит подставить и клиента, и менеджера.
        head.append(
            f"ВАЖНО: ровно по запросу клиента пусто, поэтому поиск шёл БЕЗ учёта "
            f"«{found.relaxed}». Скажи об этом прямо одной фразой и предложи "
            "посмотреть эти варианты — не выдавай их за точное совпадение."
        )
