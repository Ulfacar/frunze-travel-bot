...
#
# «За границу» из формулировки убрано 14.08.2026 по ответу владельца: рабочие визы фирма
# делает ТОЛЬКО в Кыргызстан и только на отдельном канале, а на визовом номере их нет вовсе.
# Прежний текст оставлял щель — клиент слышал «за границу не оформляем» и логично спрашивал
# про рабочую в КР, на которую этому боту отвечать нечем.
SAFE_WORKVISA_REPLY = (
    "Рабочими визами мы не занимаемся: для них нужно приглашение от работодателя, это "
    "отдельная услуга. Часто подходит другой путь: туристическая, гостевая, деловая, учебная "
    "или медицинская виза (например, США: B1/B2, F1, M1). Давайте разберём вашу ситуацию на "
    "консультации и подскажем, что реально подходит именно вам."
)

_SENTENCE_SPLIT = re.compile(r"([^.!?\n]+)([.!?\n]+|$)")

# --- эхо служебной заметки ------------------------------------------------------------
# Второй рубеж к экрану в `runner._shielded_prefix`: если модель всё-таки заговорила о
# служебном контексте, клиент не должен это увидеть. Маркеры взяты из 9 РЕАЛЬНЫХ реплик
# на проде (11.07–21.08.2026), а не придуманы: «Спасибо за служебную заметку! 👍»,
# «Это была служебная заметка для меня», «Понял! 📝 Сегодня 11 июля 2026, суббота»,
# «Готов помогать клиентам!».
#
# Ловим ТОЛЬКО мета-разговор о заметках и дословные куски инструкций. Часы приёма и дату
# бот называет клиенту законно («работаем с 9 до 19», «сегодня пятница»), поэтому сами по
# себе они маркерами НЕ являются — иначе резали бы правильные ответы.
_SERVICE_NOTE_ECHO = re.compile(
    r"служебн\w*\s+(заметк|контекст|справк|данн|информац)"
    r"|\[\s*служебная\s+заметка"
    r"|готов\s+помогать\s+клиент"
    r"|отсчитыва\w*\s+(все\s+)?относительн\w*\s+дат"
    r"|не\s+угадыва\w*\s+(текущий\s+)?(месяц|год)"
    r"|уже\s+известно\s+от\s+клиента"
    r"|клиент\s+пишет\s+по-английски"
    r"|ближайшие\s+дни,?\s+котор\w*\s+можно\s+предлагать"
    r"|доступные\s+дни\s+для\s+визитов"
    # «Спасибо! Я учёл:» — двоеточие после «учёл/запомнил» открывает дамп заметки.
    # Не привязываемся к началу строки: на проде перед ним стояло «Спасибо! ».
    r"|(^|[.!?]\s*)(я\s+)?(учёл|учел|запомнил)\s*:",
    re.IGNORECASE | re.MULTILINE,
)

# Строки самого дампа: «Сегодня 21.08.2026, пятница», «Рабочие часы: 09:00–19:00»,
# «Выходной: воскресенье». Маркеров в них нет, и по отдельности они законны — бот имеет
# полное право сказать «сегодня пятница» или «работаем до 19:00». Поэтому этот список
# применяется ТОЛЬКО внутри реплики, которая уже уличена маркером выше: там такие строки
# заведомо часть пересказанной заметки, а не ответ клиенту.
_NOTE_DUMP_LINE = re.compile(
    r"^\s*[-*•]?\s*(сегодня|дата|рабочие\s+часы|часы\s+приёма|часы\s+приема|выходной"
    r"|выходные\s+дни|работаем\s+без\s+выходных|доступные\s+дни|ближайшие\s+дни"
    r"|время\s+бишкека|язык\s+клиента)\b",
    re.IGNORECASE,
)

# Чем заменяем реплику, от которой после чистки не осталось ничего содержательного.
SAFE_SERVICE_NOTE_REPLY = "Да, конечно 🙂 Чем могу помочь — тур, авиабилеты или виза?"

# Короче этого остаток считаем мусором («Понял!», «Ок 👍») и отдаём нейтральную реплику.
_MIN_MEANINGFUL_LEN = 20


def _strip_service_note_echo(text: str) -> tuple[str, bool]:
    """Убрать строки, в которых бот проговаривает служебный контекст.

    Режем построчно, а не целиком: в реальном случае 21.08 первая строка была утечкой
    («Это была служебная заметка для меня»), а вторая — нормальным вопросом клиенту
    («Чем я могу вам помочь?»), и её надо сохранить.
    """
    if not _SERVICE_NOTE_ECHO.search(text):
        return text, False
    kept = [ln for ln in text.split("\n")
            if not _SERVICE_NOTE_ECHO.search(ln) and not _NOTE_DUMP_LINE.search(ln)]
    clean = "\n".join(kept).strip()
    clean = _MULTINL.sub("\n\n", clean)
    if len(clean) < _MIN_MEANINGFUL_LEN:
        return SAFE_SERVICE_NOTE_REPLY, True
    return clean, True
...
def validate_reply(text: str, funnel: str | None,
                   *, shield_service_notes: bool = False) -> tuple[str, list[str]]:
    ...
    `shield_service_notes` — второй рубеж экрана служебных заметок; ходит вместе с
    флагом `service_notes_shield_enabled`, поэтому по умолчанию выключен.
    """
    violations: list[str] = []
    ...
    if shield_service_notes:
        clean, echoed = _strip_service_note_echo(clean)
        if echoed:
            violations.append("service_note_echo_stripped")
