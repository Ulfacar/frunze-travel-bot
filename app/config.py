...
class Settings(BaseSettings):
    ...
    # Экран служебных заметок: дата/график/язык/реклама/квалификация подмешиваются в ход
    # с ролью `user`, то есть выглядят для модели как речь клиента. На содержательном
    # ходе это безвредно, а на пустом («Спасибо», «Хорошо», фото) модель отвечает именно
    # на заметку и пересказывает её вслух: 9 случаев с 11.07.2026, последний — 21.08 при
    # менеджере («Спасибо! Я учёл: Сегодня 21.08.2026… Готов помогать клиентам! 👍»).
    # Экран схлопывает заметки в одно сообщение и закрывает их ответом ассистента, а
    # валидатор режет утёкшее эхо. Дефолт OFF: включаем отдельным решением.
    service_notes_shield_enabled: bool = False
    ...
    # Будить только когда тишина означает поломку: Wappi принимает сообщения, а до нас
    # они не доходят. «Тихо и там, и у нас» — отсутствие клиентов, а не авария. Замер
    # 23-24.08: четыре красные тревоги за трое суток по ЗДОРОВЫМ каналам, три из них
    # сразу после ночи (порог 12 ч стоит впритык к обычной ночной паузе). Дефолт OFF.
    silence_alert_only_on_gap: bool = False
    ...
    # Карточка уходит из «Новый лид», как только с клиентом начали работать, — не дожидаясь
    # полной квалификации. Замер 07.09: 340 из 426 туровых лидов стояли в NEW, потому что
    # порог «направление + даты + туристы» проходят 11% диалогов — 85% забирает менеджер
    # раньше. У виз этой беды нет: там карточки двигают руками сами менеджеры (Медина 322
    # перевода за месяц, Элиза 242), а по турам Адеми 13, Айсина 0. Дефолт OFF.
    bitrix_stage_dialog_started_enabled: bool = False
    # Сторож самого контроллера карточек. 07.09 догоняющий проход сутки отдавал moved=0
    # при непустой очереди, и это не увидел никто: stats уходили только в log.info.
    # Пороги калибруются на истории прогонов, дефолт OFF — детектор, который пишет
    # человеку, включается отдельным решением после замера.
    pipeline_controller_alert_enabled: bool = False
    pipeline_stall_runs: int = 3             # прогонов подряд «очередь есть, движения нет»
    pipeline_silence_hours: float = 2        # столько часов без прогона = джоба встала
    pipeline_errors_per_day: int = 20        # ошибок портала за сутки, выше — поломка
    pipeline_alert_cooldown_hours: float = 12
    ...
    # Свежие факты доносятся в уже записанное досье. Замер 14.09: у 28 из 52 туровых
    # диалогов бот знал направление, а в карточке его не было — досье записалось пустым в
    # начале, а дочитывание после перехвата обновляло только нашу базу. Дефолт OFF.
    dossier_refresh_enabled: bool = False
    # Туровые поля самого портала («Какая страна ?», «Даты поездки ?», «Количество
    # туристов»). Менеджер смотрит на поля, а не в комментарий: 14.09 они были пусты во всех
    # 165 свежих карточках. Пишем только в ПУСТОЕ поле — рука человека главнее. Дефолт OFF.
    bitrix_lead_fields_enabled: bool = False
    # Досье на ОБЩЕЙ карточке Открытой линии (замер 11.09: 74 карточки, на одной 19
    # клиентов). Без подписи бот перезаписывал комментарий целиком, и менеджер читал
    ...
    # Тревога «бот не получал сообщений ~N мин» по тишине ВСЕХ каналов разом. Замер 23.09:
    # за 14 дней 45 дневных пауз клиентов дольше 30 мин (~3 в день) — обычное затишье, а
    # алерт советует «проверить Wappi/вебхуки». 23.09 в 20:34 так и вышло: вебхуки живы
    # (эхо менеджеров шло), клиенты просто молчали. Мёртвый канал ловит сторож v3 по
    # статусу профиля Wappi. Дефолт OFF: сторож сбоев работает, тишина — только по решению.
    watchdog_silence_enabled: bool = False
    ...
    # Точнее предыдущей карты: на getvisa менеджеров двое (Медина 96451, Элиза 110841),
    # и канал не говорит, чей это диалог. Логин владельца -> ID пользователя Битрикса;
    # пусто или логина нет в карте — падаем обратно на bitrix_assignee_by_bot.
    bitrix_assignee_by_manager: dict[str, str] = {}
    ...
    # Подборка туров карточками, как её присылают менеджеры (встреча 11.08). Дефолт OFF —
    # выкатываем, ничего не меняя. Публичный адрес нужен для ссылки «Подробнее здесь»:
    # пусто → карточки уйдут без ссылки, это не авария.
    tours_cards_enabled: bool = False
    public_base_url: str = ""
    ...
    # человека. Default OFF; рантайм-ключ `awaiting_telegram_enabled`. Сторож на
    # WhatsApp-номер команды существовал и не сработал ни разу: ALERT_WHATSAPP_TO
    # на проде пуст (замер 11.09), а личный телеграм менеджеров уже работает.
    awaiting_telegram_enabled: bool = False
    # Старше этого не напоминаем: лид недельной давности уже мёртв, звонок по нему
    # только злит менеджера (то же правило, что в догоняющей отправке заявок).
    awaiting_max_age_hours: int = 24
    # Сколько раз всего напоминать по одному ожиданию. Замер первых суток: 279
    # напоминаний на 36 диалогов, рекорд 23 по одному. Менеджер приходил в 16 случаях
    # из 36 — значит работает первое-второе, а дальше это уже не напоминание.
    awaiting_max_pings: int = 2
    # Бот молча дочитывает переписку и дозаполняет карточку, пока диалог ведёт менеджер.
    # Клиенту при этом не пишет и стадию не двигает. Default OFF; рантайм-ключ
    # `facts_when_silent_enabled`, есть per-bot вариант — обкатываем по одному каналу.
    facts_when_silent_enabled: bool = False
    # Писать ли в карточку бюджет, названный в разговоре. Отдельно от остальных полей:
    # ошибка в бюджете дороже — раньше он уезжал в сумму сделки. Default OFF; рантайм-ключ
    # `tour_facts_budget_enabled`, есть per-bot вариант.
    tour_facts_budget_enabled: bool = False
    ...
    # Предохранители веерной рассылки. Сухой прогон 07.09.2026 на боевых данных: без них
    # первое включение дожима отправило бы 818 сообщений залпом по всей базе за месяцы.
    # Вечерний вопрос менеджеру «клиент оплатил?». Кнопка в панели живёт с июля и ей
    # воспользовались 1-3 раза за 90 дней: в момент продажи менеджер в WhatsApp, а не
    # в админке. Отсюда «Продано: 0» в еженедельной сводке владельцу при 251 обращении.
    # Класс C (пишем людям + правим боевой CRM), дефолт OFF.
    sale_check_enabled: bool = False
    sale_check_hour: int = 18            # час по Бишкеку, когда спрашиваем
    sale_check_max_items: int = 5        # диалогов в одном сообщении
    sale_check_min_age_hours: int = 12   # моложе — разговор ещё идёт, спрашивать рано
    sale_check_max_age_days: int = 14    # старше — менеджер уже не помнит разговор
    sale_check_snooze_days: int = 3      # «клиент думает» — через сколько спросить снова
    # Кому вечерний вопрос уходит. Пусто = ВСЕМ менеджерам с телеграмом (на 10.09 их
    # пятеро, включая визовых и заказчика). Класс C обкатывается на одном человеке,
    # поэтому список задаётся явно, а не «повезёт, что у остальных нет туровых диалогов».
    sale_check_managers: list[str] = []
    followup_batch_limit: int = 20        # сколько касаний уходит за один прогон
    followup_max_age_days: int = 14       # молчит дольше — не дожимаем, это уже спам
