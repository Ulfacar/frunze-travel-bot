...
FEATURE_FLAGS = {
    ...
    "bitrix_stage_dialog_started_enabled": {
        ...
        "desc": ("Бот двигал стадию только по полной квалификации — направление, даты и "
                 ...
                 "воронке взять было неоткуда. С тумблером карточка уезжает в «Выявление "
                 "потребностей», как только с клиентом реально начали работать: менеджер "
                 "перехватил, диалог закреплён, собран хотя бы один факт или клиенту "
                 "ответили. Никто не ответил — карточка честно остаётся в «Новом». "
                 "«Переписка/Недозвоны» и касания бот не занимает: это ручные статусы "
                 "менеджеров. Ручной перенос по-прежнему замораживает бота."),
