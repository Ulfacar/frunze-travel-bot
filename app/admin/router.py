...
def _sale_check_note() -> str:
    """Честно сказать, чего не хватает: молчаливый тумблер уже стоил нам инцидента."""
    if not settings.public_base_url:
        return "⚠️ Не задан PUBLIC_BASE_URL — ссылки некуда вести."
    if not (settings.webhook_secret or "").strip():
        return "⚠️ Пустой WEBHOOK_SECRET — подписать ссылку нечем, вопрос не уйдёт."
    if not settings.bitrix_autodeal_enabled:
        return ("ℹ️ «Заводить сделку» выключен: лид уедет в «Подписан», но сделки "
                "в воронке FrunzeTravel не будет.")
    return ""
...
FEATURE_FLAGS = {
    ...
    "bitrix_stage_catchup_enabled": {
        "title": "Двигать карточку, даже когда диалог ведёт менеджер",
        "desc": ("Раньше стадия лида замирала, как только менеджер вступал в переписку: "
                 "бот считал, что раз он замолчал, то и карточку трогать нельзя. По турам "
                 "так стояло 64 карточки из 72 — факты клиента собраны, а лид висит в "
                 "«Новый». Тумблер разделяет две разные вещи: менеджер ПИШЕТ клиенту и "
                 "менеджер ДВИГАЕТ карточку. Если стадию меняли руками, бот по-прежнему "
                 "замирает и назад карточку не тянет. Плюс раз в десять минут догоняет "
                 "стадии по диалогам, где факты уже собраны."),
    ...
    "bitrix_deal_contact_enabled": {
        "title": "Привязывать клиента к сделке в Битриксе",
        "desc": ("В Битриксе телефон живёт у контакта, а не у сделки: без контакта "
                 "менеджер не может позвонить прямо из карточки. Замер 11.09: из пяти "
                 "сделок в воронке FrunzeTravel контакт есть у двух — тех, что завели "
                 "руками. С тумблером бот ищет контакт по телефону клиента, а если его "
                 "в портале нет — заводит с именем и номером. Это НОВЫЕ записи в CRM "
                 "заказчика, поэтому решение за ним. Выключено — сделка всё равно "
                 "создаётся, просто без телефона."),
        "default": lambda: settings.bitrix_deal_contact_enabled,
        "note": lambda: ("" if settings.bitrix_autodeal_enabled
                         else "ℹ️ «Заводить сделку» выключен — привязывать пока некуда."),
    },
    "sale_check_enabled": {
        "title": "Спрашивать менеджера вечером: «клиент оплатил?»",
        "desc": ("Кнопка «Оплатил» в панели живёт с июля — ей воспользовались 1-3 раза "
                 "за 90 дней. Не из-за лени: в момент продажи менеджер в WhatsApp, а не "
                 "в админке. Из-за этого владелец пятую неделю подряд видит в сводке "
                 "«Продано: 0» при 251 обращении за неделю, а в воронке FrunzeTravel "
                 "лежат 5 сделок за три месяца. С тумблером бот раз в день вечером "
                 "присылает менеджеру список его диалогов, где работа шла, а исход не "
                 "отмечен — по две ссылки на каждый: «Оплатил» и «Не сложилось». Одно "
                 "отмечен — по три ссылки на каждый: «Оплатил», «Не сложилось» и "
                 "«Ещё думает». Одно касание, заходить никуда не нужно; ссылка "
                 "открывает страницу с кнопкой, само открытие ничего не записывает. "
                 "Отметка «Оплатил» ведёт лид в «Подписан» — а сделка в воронке "
                 "появится, только если включён тумблер «Заводить сделку». Не больше "
                 "пяти диалогов в сообщении и один раз в сутки на менеджера."),
        "default": lambda: settings.sale_check_enabled,
        "note": lambda: _sale_check_note(),
    },
    ...
    "bitrix_stage_dialog_started_enabled": {
        ...
        "desc": ("Бот двигал стадию только по полной квалификации — направление, даты и "
                 "состав туристов. Этот порог проходят 11% туровых диалогов, потому что "
                 "85% забирает менеджер раньше, чем бот успевает выяснить. Итог: 340 из "
                 "426 туровых лидов за месяц стояли в «Новом лиде», и статистику по "
                 "воронке взять было неоткуда. С тумблером карточка уезжает в "
                 "«Переписка/Недозвоны» — туда же, куда визовые менеджеры переносят "
                 "карточки руками, — как только с клиентом реально начали работать: "
                 "менеджер перехватил, диалог закреплён или клиенту ответили. Никто не "
                 "ответил — карточка честно остаётся в «Новом». В «Выявление "
                 "потребностей» карточка едет отдельно и только тогда, когда потребность "
                 "ДЕЙСТВИТЕЛЬНО выяснена (направление, даты, состав): 09.09 проверка "
                 "показала, что из 281 карточки в этой стадии 264 были пустыми, и стадия "
                 "врала менеджеру. Ручной перенос по-прежнему замораживает бота."),
        "default": lambda: settings.bitrix_stage_dialog_started_enabled,
        "note": lambda: ("" if (settings.bitrix_stage_map or {}).get("dialog_started")
                         else "⚠️ В BITRIX_STAGE_MAP нет ключа dialog_started — двигать некуда."),
    },
    "dossier_when_intercepted_enabled": {
        "title": "Вести досье в карточке, пока диалог ведёт менеджер",
        "desc": ("Требование заказчика по турам: бот заполняет карточку сам, менеджер лишь "
                 "сверяется с клиентом, когда тот пришёл в офис. Раньше бот переставал вести "
                 "сводку, как только менеджер вступал в переписку, — а перехвачено 8 из 11 "
                 "туровых диалогов, то есть досье не появлялось почти нигде. Текст, "
                 "написанный человеком, бот не трогает: увидел в поле чужую правку — "
                 "оставил как есть."),
        "default": lambda: settings.dossier_when_intercepted_enabled,
        "note": lambda: "",
    },
    "dossier_refresh_enabled": {
        "title": "Обновлять досье в карточке, когда клиент назвал новое",
        "desc": ("Досье записывалось один раз — часто в самом начале, пустым, — и больше не "
                 "менялось: клиент потом называл страну и даты, бот клал их в нашу базу, а в "
                 "Битрикс они не доезжали. Замер 14.09: у 28 из 52 клиентов бот знал "
                 "направление, а в карточке его не было. Стадию это не двигает; текст, "
                 "дописанный менеджером, по-прежнему не трогается."),
        "default": lambda: settings.dossier_refresh_enabled,
        "note": lambda: "",
    },
    "bitrix_lead_fields_enabled": {
        "title": "Заполнять поля карточки: страна, даты, туристы",
        "desc": ("Менеджер смотрит в поля «Какая страна ?», «Даты поездки ?», «Количество "
                 "туристов», а не в комментарий — 14.09 они были пусты во всех 165 свежих "
                 "туровых карточках. Бот пишет только в ПУСТОЕ поле: что вписал человек, то "
                 "и остаётся. Общие карточки Открытой линии (много клиентов на одной) не "
                 "трогает."),
        "default": lambda: settings.bitrix_lead_fields_enabled,
        "note": lambda: "",
    },
    "dossier_shared_cards_enabled": {
        "title": "Подписывать досье на общих карточках номером клиента",
        "desc": ("Открытая линия иногда сажает на одну карточку нескольких клиентов — "
                 "замер 11.09: таких карточек 74, на одной 19 человек. Бот перезаписывал "
                 "комментарий целиком, и менеджер читал сводку последнего написавшего как "
                 "сводку про всю карточку. С тумблером каждая сводка подписана номером, "
                 "чужие сохраняются; показываем 10 свежих клиентов. Обычных карточек "
                 "(один клиент) это не касается."),
    ...
    "llm_fallback_handoff_enabled": {
        ...
        "desc": ("Когда ход не удался (у LLM кончились деньги, упал инструмент), бот "
                 "отвечал «Секундочку, уточню детали и вернусь к вам». 17-22.09 так "
                 "случилось 178 раз за шесть дней, и никто не вернулся. Хуже того, эта "
                 "отписка считалась ответом: диалог выглядел отвеченным, и сторож "
                 "«клиент ждёт» его не видел. С тумблером бот честно говорит про "
                 "заминку и диалог остаётся НЕотвеченным: он виден в панели и в утреннем "
                 "горячем листе как ждущий ответа. Живой пинг менеджеру уходит только "
                 "по диалогам, которые уже у человека (перехвачены или стадия «менеджер») "
                 "— на аварии 17-22.09 это было бы 185 случаев из 328; остальные 143 "
                 "висели в приветствии и видны только в панели."),
        "default": lambda: settings.llm_fallback_handoff_enabled,
        "note": lambda: "",
    },
    "openrouter_balance_check_enabled": {
        ...
        "desc": ("Предупреждает заранее: «осталось столько-то, хватит примерно на N дней». "
                 "Порог считается в днях, а не в долларах — при туровом трафике деньги "
                 "уходят примерно вдесятеро быстрее, чем при визовом. 21.08 бот замолчал "
                 "на исходе счёта, и узнали об этом от клиента."),
        "default": lambda: settings.openrouter_balance_check_enabled,
        "note": lambda: ("" if settings.openrouter_api_key
                         else "⚠️ Нет ключа OpenRouter — спрашивать баланс нечем."),
    },
    "silence_alert_only_on_gap": {
        "title": "Тревожить тишиной только при поломке",
        "desc": ("Раньше сторож писал «канал молчит 12 часов» и по здоровым каналам: "
                 "клиенты просто спят, а порог стоит впритык к обычной ночи — три "
                 "красных сообщения из четырёх приходили сразу после неё. С тумблером "
                 "будим только когда Wappi принимает сообщения, а до нас они не доходят: "
                 "это настоящая поломка. «Тихо и там, и у нас» — отсутствие клиентов, "
                 "а не авария."),
        "default": lambda: settings.silence_alert_only_on_gap,
        "note": lambda: "",
    },
    "bitrix_health_enabled": {
        "title": "Следить за доступностью Битрикса",
        "desc": ("Сообщает, если портал перестал принимать запросы бота: карточки и "
                 "переписка в это время копятся у нас. Одиночный сбой пропускается — "
                 "тревога только после нескольких неудач подряд."),
        "default": lambda: settings.bitrix_health_enabled,
        "note": lambda: ("" if settings.bitrix24_webhook_url
                         else "⚠️ Вебхук Битрикса не настроен."),
    },
    "tour_facts_enabled": {
    ...
    "followup_enabled:tours": {
        "title": "Автодожим — только по турам",
        "desc": ("Дожим отдельно по воронкам: у туров и виз разные заказчики и разные номера. "
                 "Этот тумблер включает дожим ТОЛЬКО туровым клиентам. Замер 15.09: в очереди "
                 "42 туровых диалога, молчащих от суток до двух недель; уходят порциями по 20 "
                 "каждые 5 минут и только с 09:00 до 22:00 по Бишкеку. Пока выключены и он, и "
                 "общий тумблер — по турам не уходит ничего."),
        "default": lambda: settings.followup_enabled,
        "note": lambda: "",
    },
    "followup_enabled:visa": {
        "title": "Автодожим — только по визам",
        "desc": ("То же самое для визовой воронки (номер GetVisa, заказчик Гриша). Замер 15.09: "
                 "в очереди 77 визовых диалогов — это 77 сообщений его клиентам, поэтому "
                 "включать только с его ведома."),
        "default": lambda: settings.followup_enabled,
        "note": lambda: "",
    },
    "followup_enabled:tickets": {
        "title": "Автодожим — только по билетам",
        "desc": ("То же для воронки авиабилетов. Воронка тонкая, очередь обычно пустая."),
        "default": lambda: settings.followup_enabled,
        "note": lambda: "",
    },
    "dozhim_enabled": {
    ...
    "service_notes_shield_enabled": {
        "title": "Не проговаривать служебные заметки клиенту",
        "desc": ("Боту на каждый ход подкладывают служебные справки — сегодняшняя дата, "
                 "часы приёма, язык клиента, текст объявления. Технически они приходят "
                 "как будто их написал клиент, и на пустой ход («Спасибо», «Хорошо», "
                 "фото) бот отвечает на справку, а не человеку: «Спасибо! Я учёл: "
                 "сегодня 21.08.2026, рабочие часы 09:00–19:00… Готов помогать "
                 "клиентам!». Так было 9 раз с 11 июля, в последний — при менеджере. "
                 "Тумблер прячет справки за рамку «это не речь клиента» и вырезает из "
                 "ответа проговорки, если они всё же случились."),
        "default": lambda: settings.service_notes_shield_enabled,
        "note": lambda: "",
    },
    "stt_guard_enabled": {
