...
async def note_run(stats: dict) -> None:
    ...
    try:
        ...
        # Простой — это когда очередь есть, а сделано НИЧЕГО. Калибровка на первых четырёх
        # боевых прогонах: сдвинуто 28, а сводок 84 — слоты уходят и на запись досье в
        # карточки с уже правильной стадией. По правилу «нет движений = простой» такой
        # прогон выглядел бы застреванием, и владельцу ушла бы тревога о работающем
        # контроллере. Именно так 07.08 сгорел сторож каналов: 5.3 ложных в сутки, и его
        # отключили. Пустая очередь тоже не простой — делать было нечего.
        did_work = moved > 0 or int(stats.get("dossiers", 0) or 0) > 0
        if waiting > 0 and not did_work:
