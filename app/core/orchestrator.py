...
# Отметка в ПАНЕЛИ (не клиенту): распознавание запускали и оно не дало текста. Клиент в
# этом случае получает прежний fallback — сомнительную расшифровку боту не отдаём.
NON_TEXT_VOICE_FAILED = "🎤 Голосовое — не удалось распознать, прослушайте запись"
...
class Orchestrator:
    ...
    async def _run_turn(self, msg: Message) -> None:
        ...
        # Успешно распознанный голос идёт текстовым путём и сам сбрасывает серию: бот его
        # понимает, поэтому авто-хендофф после двух голосовых больше не нужен.
        state.consecutive_audio = 0
