...
async def seed_defaults() -> None:
    ...
    defaults = [
        ...
        },
        {
            ...
            "enabled": True,
            "priority": 42,
            "title": "Авиабилеты — к менеджеру",
            # Авиабилеты бот не подбирает (нет GDS): спрашиваем маршрут+дату+пассажиров и
            # передаём менеджеру — ответ клиента увидит уже он (правка со встречи 06.07).
            "patterns": [
                "авиабилет",
                "авиа билет",
                "билет на самолет",
            ...
            # Не перехватываем, если билет — часть тур-запроса (тур/путёвка/отель/включено):
            # такой диалог бот должен вести сам, а не глушить необратимым хендоффом.
            "negative_terms": ["входн", "экскурс", "музе", "концерт", "матч",
                               "тур", "путевк", "отел", "включ", "виз"],
            "answer": (
                "По авиабилетам вас сориентирует менеджер ✈️ Напишите, пожалуйста, направление "
                "(откуда–куда), дату вылета и сколько пассажиров — передаю менеджеру, он подберёт "
                "рейс и пришлёт цену."
            ),
            "handoff_only": True,
            "allow_during_qualification": True,
        ...
        },
        {
            ...
            "negative_terms": [],
            # Тот же текст, что и SAFE_WORKVISA_REPLY в валидаторе: FAQ отвечает раньше LLM,
            # и расхождение между ними означало бы два разных ответа на один вопрос.
            "answer": (
                "Рабочие визы за границу мы не оформляем: для них нужно приглашение от "
                "работодателя, это отдельная услуга. Часто подходит другой путь: "
                "туристическая, гостевая, деловая или учебная виза (например, США: B1/B2, "
                "F1, M1). Давайте разберём вашу ситуацию на консультации и подскажем, что "
                "реально подходит именно вам."
            ),
            "handoff_only": False,
            "allow_during_qualification": True,
        },
        {
            ...
            "negative_terms": [],
            "answer": (
                "Сроки зависят от страны: Шенген ~15–45 дней, Великобритания ~3–8 недель, "
                "Канада ~1–6 месяцев, США — от нескольких дней после собеседования, "
                "Япония ~5–10 дней, Корея ~2–4 недели, Китай ~4–10 дней. "
        ...
        },
        {
            ...
            "negative_terms": [],
            "answer": (
                f"GetVisa (визовый отдел): {GETVISA_OFFICE_ADDRESS}. "
                f"Часы работы: {GETVISA_WORKING_HOURS}."
            ),
            "handoff_only": False,
            "allow_during_qualification": True,
        },
        {
            ...
            "negative_terms": [],
            "answer": (
                "Работаем по многим направлениям — США, Канада, Шенген, Великобритания, "
                "Китай, Таиланд, Япония, Южная Корея, ОАЭ и другим. По туристическим, "
                "гостевым и деловым визам. По какой стране вас интересует?"
            ),
            "handoff_only": False,
            "allow_during_qualification": True,
        },
        {
            ...
            "patterns": [
                ...
                "что входит в услугу",
                "чем помогаете",
                "какая помощь по визе",
                "что за услуга",
            ],
            "negative_terms": [],
            "answer": (
                "В сопровождение входит: персональный кейс, заполнение анкеты, запись на "
                "подачу/интервью, подготовка к собеседованию и помощь на этапах оформления. "
                "Консульские и сервисные сборы оплачиваются отдельно."
            ),
            "handoff_only": False,
            "allow_during_qualification": True,
        },
        {
            ...
            "enabled": True,
            "priority": 20,
            "title": "Страна вне данных — к менеджеру",
            "patterns": [
                "бразил",
                "индия",
                "мексик",
            ...
            "negative_terms": [],
            "answer": (
                "По этому направлению вас сориентирует визовый эксперт — передаю менеджеру, "
                "он подскажет по срокам и документам."
            ),
            "handoff_only": True,
            "allow_during_qualification": True,
        },
        {
            ...
            "negative_terms": [],
            "answer": (
                "Срочность зависит от страны и доступных слотов записи — менеджер проверит "
                "актуальные варианты по вашему направлению."
            ),
            "handoff_only": False,
            "allow_during_qualification": True,
        },
        {
            ...
            "negative_terms": [],
            "answer": "По порядку и способам оплаты менеджер подскажет актуальные варианты.",
            "handoff_only": False,
            "allow_during_qualification": True,
        },
        {
            ...
            "enabled": True,
            "priority": 15,
            "title": "Документы для визы (общее)",
            "patterns": [
                "какие документы",
                "что нужно для визы",
                "список документов",
            ...
            "negative_terms": [],
            "answer": (
                "Обычно нужны загранпаспорт, фото, анкета, бронь/маршрут, страховка, "
                "справка с работы/учёбы и финансовые документы. Точный список зависит от "
                "страны и вашей ситуации — менеджер подскажет набор под вас."
            ),
            "handoff_only": False,
            "allow_during_qualification": True,
        },
