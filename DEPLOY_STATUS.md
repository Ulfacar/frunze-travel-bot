...
## Релизная проверка (smoke на прод-образе)

Перед деплоем прогнать на docker-хосте (ловит «нет зависимости в образе / роут не собран»):

```bash
bash scripts/smoke.sh
```

Поднимает собранный образ изолированно (memory-бэкенды, без БД), бьёт /health,
вебхук Wappi (echo+delivery), логин формой (проверяет python-multipart), доски и
/admin/analytics. Ожидаемо: `SMOKE PASSED`.

## Админ-панель — возможности (после доработок 24.06.2026)

- Вход по логину менеджера (`MANAGERS` в env), сессия-cookie; «Выйти».
- Канбан по воронкам; метрика «Требуют ответа» + фильтр «ваш ход»; бейджи ожидания.
- Карточка: AI-бриф, перехват/возврат, «кто ведёт» + предупреждение если занято.
- Ответ из браузера со статусом доставки (⏳/✓/✓✓/✗) и кнопкой «Повторить».
- Быстрые ответы по воронке + «✨ Предложить ответ» (черновик от Claude).
- Кнопки исхода (Оплатил/Дошёл/Слился); авто-исход из стадии.
- `/admin/analytics`: containment, исходы, воронки, время ответа/перехвата, причины эскалаций.
- `/health` отдаёт `last_inbound_seconds_ago` (детектор «тишины» вебхуков).
